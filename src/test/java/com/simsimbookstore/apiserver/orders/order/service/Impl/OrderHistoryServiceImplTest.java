...
class OrderHistoryServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        mockUser = User.builder().userName("Test User").build();
        mockOrder = Order.builder()
                .orderNumber("ORD123")
                .orderDate(LocalDateTime.now())
                .user(mockUser)
                .delivery(null) // Delivery가 null인 경우
    ...
    void getOrderHistoryWithNullDelivery() {
        ...
        // 서비스 호출
        Page<OrderHistoryResponseDto> result = orderHistoryService.getOrderHistory(1L, PageRequest.of(0, 10));

        // 결과 검증
        assertEquals(1, result.getTotalElements());
        OrderHistoryResponseDto dto = result.getContent().getFirst();
        assertNull(dto.getTrackingNumber(), "Tracking number should be null");
        assertEquals("Unknown Receiver", dto.getReceiverName(), "Receiver name should be default");
